...
// FIXED: Socket.IO connection handler with proper async/await
io.on('connection', async (socket) => {   
  ...
  // 🆕 FIXED: Get online users and broadcast user coming online
  try {
  ...
  // 🆕 Handle explicit away/busy status
  socket.on('setAway', () => {
    console.log(`😴 User ${socket.user.id} is now away`);
    socket.broadcast.emit('userStatusUpdate', {
      userId: socket.user.id,
      status: 'away',
      lastSeen: null
  ...
  socket.on('setBusy', () => {
    console.log(`🔴 User ${socket.user.id} is now busy`);
    socket.broadcast.emit('userStatusUpdate', {
      userId: socket.user.id,
      status: 'busy',
      lastSeen: null
  ...
  socket.on('setOnline', () => {
    console.log(`🟢 User ${socket.user.id} is back online`);
    socket.broadcast.emit('userStatusUpdate', {
      userId: socket.user.id,
      status: 'online',
      lastSeen: null
  ...
  // 🆕 Get user status (for when someone wants to check)
    ...
    try {
      ...
      if (userSocket) {
        callback({
          ...
          data: {
            userId,
            status: 'online', // or get from database if you store custom status
            lastSeen: null
          }
        });
      } else {
        // User is offline, get last seen from database
        // const user = await User.findById(userId, 'lastSeen status');
        callback({
          status: 'success',
          data: {
            userId,
            status: 'offline',
            lastSeen: new Date().toISOString() // Replace with user.lastSeen from DB
          }
        });
      }
    } catch (err) {
