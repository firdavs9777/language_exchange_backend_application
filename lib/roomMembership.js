...
/**
 * Ban check for topic-room join gating (Task 16 — moderation). Hubs have no
 * ban concept — callers are responsible for gating on roomType === 'topic'
 * before relying on this (a hub's bannedUsers array should always be empty
 * in practice, but this function doesn't assume that itself).
 *
 * @param {String|Object} userId
 * @param {Object} hub - must have `bannedUsers` (array)
 * @returns {boolean}
 */
function isBanned(userId, hub) {
  return includesId(hub?.bannedUsers, userId);
}

/**
 * Whether `userId` currently has a pending join request recorded on `hub`.
 * @param {String|Object} userId
 * @param {Object} hub - must have `joinRequests` ([{ user }])
 * @returns {boolean}
 */
function hasPendingJoinRequest(userId, hub) {
  if (!Array.isArray(hub?.joinRequests)) return false;
  return hub.joinRequests.some((r) => r && idsEqual(r.user, userId));
}

/**
 * Decide the effect of POST /rooms/:id/request-join, before any DB write.
 * The controller re-checks this atomically at write time (a conditional
 * $push guarded by the same two conditions) so a race between two
 * concurrent requests can't create a duplicate pending entry — this
 * function only produces the friendly pre-check error reason.
 *
 * @param {String|Object} userId
 * @param {Object} hub - must have `participants`, `joinRequests`
 * @returns {{ ok: boolean, reason: 'already-member'|'already-pending'|'ok' }}
 */
function decideRequestJoin(userId, hub) {
  if (includesId(hub?.participants, userId)) {
    return { ok: false, reason: 'already-member' };
  }
  if (hasPendingJoinRequest(userId, hub)) {
...
/**
 * Owner-kick guard (Task 16 — moderation): the room owner can never be
 * kicked/banned via removeMember, even by another admin. Returns true when
 * kicking `targetUserId` is allowed.
 *
 * @param {String|Object} targetUserId
 * @param {Object} hub - must have `owner`
 * @returns {boolean}
 */
function canKickMember(targetUserId, hub) {
  if (!hub) return false;
  return !idsEqual(hub.owner, targetUserId);
}
