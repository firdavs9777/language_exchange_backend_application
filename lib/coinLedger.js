...
async function credit(userId, amount, { type = 'purchase', reason = '', metadata = {} } = {}) {
  ...
  try {
    ...
    if (isDuplicateIapTransactionError(err) && iapTransactionId) {
      // Scope the lookup to THIS user too — a genuine retry of the same
      // purchase is always by the same user (the idempotency id is a
      // per-purchase store identifier). If the existing row belongs to a
      // different user, this isn't a legitimate replay — it's an
      // unexpected conflict, so fall through and rethrow rather than
      // silently handing back someone else's transaction.
